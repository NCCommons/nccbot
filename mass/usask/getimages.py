...
def extract_images_from_url(url):
    ...
    for n, figure_tag in enumerate(figure_tags):
        logger.info(f"\t<<purple>> >> figure {n+1}/{len(figure_tags)}")
        ...
        logger.info(f"\t\t <<yellow>> caption: <<default>> {caption}")
        ...
        if img_tag:
            ...
            # logger.info(f'\t\t <<yellow>> img_srcset: <<default>> {img_srcset}')
            # logger.info(f'\t\t <<yellow>> img_src: <<default>> {img_src}')
            # Split the 'srcset' value by comma and get the first URL
            img_url = img_srcset
            if not img_srcset:
                img_url = img_src
                logger.info(f"\t\t <<red>> no srcset, use src")

            # Remove the dimension part from the URL using regex
            img_url = re.sub(r"-\d+x\d+(\.\w+)$", r"\1", img_url)
            logger.info(f"\t\t <<yellow>> img_url: <<default>> {img_url}")
...
def main():
    ...
    for section, section_data in data.items():
        ...
        logger.info(f"<<yellow>> Processing section {n}/{len(data)}: {section}")
