...
def update_text(title, MAIN_CAT_ONE, files=0):
    # ---
    logger.info(f"<<yellow>> update_text: {files=}")
    # ---
...
def mk_ids_titles():
    ...
    # ---
    logger.info(f"<<yellow>> ids work_get_files_data: {len(ids):,}")
    logger.info(f"<<yellow>> ids from studies_titles: {len(ids_to_titles):,}")
    # ---
...
def check_cat_page(MAIN_COUNT, MAIN_CAT_ONE):
    ...
    if not cat_page.exists():
        logger.info(f"<<red>> {MAIN_CAT_ONE} not exists")
...
def do_titles(ids_titles, MAIN_COUNT, MAIN_CAT_ONE):
    ...
    # ---
    logger.info(f"<<yellow>> titles_only_one: {len(ids_titles):,}")
    logger.info(f"<<yellow>> titles_only_one: {len(ids_titles):,}")
    logger.info(f"<<yellow>> titles_only_one: {len(ids_titles):,}")
    # ---
    if not ids_titles:
        logger.info("<<red>> no ids_titles")
        return
    # ---
    ...
    # ---
    logger.info(f"<<green>> ids_titles: {len(ids_titles):,}, after remove already_done..")
    # ---
    if not ids_titles:
        logger.info("<<red>> no ids_titles")
        return
...
def main(ids_titles, MAIN_COUNT):
    ...
    for n, (study_id, study_title) in enumerate(ids_titles.items()):
        # ---
        logger.info(f"page: {n}/{len(ids_titles):,}:")
    ...
    if not ta:
        for x, ba in args_na.items():
            if str(x) in sys.argv:
                ta.append(x)
                logger.info(f"<<yellow>> MAIN_CAT_ONE: {ba}, MAIN_COUNT: {x}")
    # ---
    if not ta:
        ta = [1]
    # ---
